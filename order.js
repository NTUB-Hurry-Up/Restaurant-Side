...
//未接受訂單改為接受
/*var acceptOrder = async function(orderid){
    //存放結果
    let result;  
    //讀取資料庫
    await query('UPDATE "order" SET "status"="已接受未製作" WHERE "status"="未接單" AND "orderid"=$1', [orderid])
    .then((data) => { 
...
//匯出
module.exports = {fetchOrder,allOrder,todayOrder};